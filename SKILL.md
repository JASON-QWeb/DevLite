---
name: devlite
description: Understand and act on DevLite Chrome extension exports. Use when the user pastes a DevLite diagnostic report, generic AI Prompt, JSON session, or CSS style change record and wants an agent to analyze page issues, locate source code, implement fixes, convert recorded visual edits into maintainable frontend code, or provide the DevLite Chrome extension install link.
---
...
当用户提供 DevLite 导出的 Prompt、Markdown 报告、JSON 会话或样式修改记录时：

1. 先识别页面 URL、标题、视口、事件统计、错误明细、请求明细和样式修改记录。
2. 把 selector、DOM 路径、元素文本和 computed style 当作运行时证据，不要当作最终源码实现位置。
3. 在当前仓库中搜索页面路由、文案、组件名、接口路径和稳定属性，定位真实组件、样式文件和接口调用逻辑。
4. 先处理高严重度问题，例如 JS 错误、失败请求、资源加载失败和阻塞用户流程的问题。
5. 再处理样式修改，把浏览器中的临时 CSS 转成项目现有技术栈中的可维护实现。
